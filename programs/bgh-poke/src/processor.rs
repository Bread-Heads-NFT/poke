...
use crate::error::BghPokeError;
use crate::instruction::accounts::CreateAccounts;
use crate::instruction::{CreateArgs, BghPokeInstruction};
...
pub fn process_instruction<'a>(
    ...
    let instruction: BghPokeInstruction =
        BghPokeInstruction::try_from_slice(instruction_data)?;
    match instruction {
        BghPokeInstruction::Create(args) => {
...
fn create<'a>(accounts: &'a [AccountInfo<'a>], args: CreateArgs) -> ProgramResult {
    ...
    if *ctx.accounts.system_program.key != system_program::id() {
        return Err(BghPokeError::InvalidSystemProgram.into());
    }
